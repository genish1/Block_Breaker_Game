...
// Fonction pour initialiser les briques et augmenter la vitesse de la balle
void ResetLevel(GameData& G, int pLevel)
{
    ...
    if (pLevel == 2)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            for (int j = 0; j < 2; j++)
            {
                Brique newBrique(V2(startX + i * 110, startY + j * 40), 1);
                newBrique.InitTextures();
                G.briques.push_back(newBrique);
            }
        }
...
void Render(const GameData& G)
{
    ...
    if (G.isPaused)
    {
        ...
        // Ajouter ce code juste après l'affichage du score final :
        string restartText = "Press 'R' to Restart";
        G2D::drawStringFontMono(V2(G.WidthPix / 2 - 200, G.HeighPix / 2 + 120), restartText, 25, 3, Color::White);
    ...
    //dessine le fond
    for (int i = 0; i < G.WidthPix / 32 + 1; ++i)
    {
        for (int j = 0; j < G.HeighPix / 32 + 1; ++j)
        {
    ...
    //dessine le score et les vies
    string scoreText = "Score: " + to_string(G.score);
    G2D::drawStringFontMono(V2(10, 10), scoreText, 20, 3, Color::White);

    G2D::drawStringFontMono(V2(20, 40), "Vies :" + to_string(G.lives), 20, 3, Color::White);
...
void Logic(GameData& G)
{
    ...
    for (int i = 0; i < 4; ++i) {  
        ...
        if (collisionType == 1) { 
            ...
            if (i == 0) { 
                ...
                float bounceAngle = normalizedRelativeIntersectionX * (75.0f * 3.14159f / 180.0f);
    ...
    {
        ResetLevel(G, 2);  // Réinitialiser le niveau et augmenter la vitesse de la balle
    }
    ...
    if (G.isPaused && G2D::isKeyPressed(Key::R)) {
        G.aBall = Balle();
        G.aBall.InitTextures();
...
int main(int argc, char* argv[])
{
    ...
    G2D::initWindow(V2(G.WidthPix, G.HeighPix), V2(20, 20), string("BLOCK BREAKER"));
